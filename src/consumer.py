...
from utils.database.redis_connect import RedisDB
...
class Consumer:
    ...
    QUEUE = 'seagame'
    pages = RedisDB().get_connection().get("pages")
    if pages:
        pages = json.loads(pages)
        ROUTING_KEY = ['webhook.{}'.format(
    ...
    def __on_connection_closed(self, connection, reply_code, reply_text):
        ...
        if self._closing:
            ...
            log_warning('Connection closed, reopening in 5 seconds: ({}) {}'.format(
    ...
    def __on_channel_closed(self, channel, reply_code, reply_text):
        ...
        """
        log_warning('Channel {} was closed: ({}) {}'.format(
